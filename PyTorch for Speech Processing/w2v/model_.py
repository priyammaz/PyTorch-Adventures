...
m = Wav2Vec2ForPreTraining(config)
print(m)

# from utils import _get_feature_vector_attention_mask
...
#         ### Sanity Checks ###
#         assert config.hidden_size % config.num_attention_heads == 0, "Double check embedding dim divisible by number of heads"
...
#             if (not self.training) or (self.training and (dropout_probability >= self.config.layerdrop)):
...
#         if config.mask_time_prob > 0.0:
#             self.masked_spec_embed = nn.Parameter(torch.FloatTensor(config.hidden_size))
...
#                 self.config, extract_features.shape[1], attention_mask
#             )
